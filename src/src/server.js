...
import {
  sign, register, login, publicUser, userFromToken, setPassword, checkPassword, tempPassword,
...
app.post('/api/password', requireUser, async (req, res) => {
  try {
    const okNow = await checkPassword(req.user, (req.body || {}).current);
    if (!okNow) return bad(res, 401, 'Current password is wrong');
    await setPassword(req.user, (req.body || {}).next);
    ok(res, {});
  } catch (e) { bad(res, 400, e.message); }
});

/* Admin issues a temporary password when a member is locked out. There is no
   email service here, so you read it back to them yourself. */
app.post('/api/admin/reset-password', requireAdmin, async (req, res) => {
  try {
    const u = store.findUserByEmail((req.body || {}).email);
    if (!u) return bad(res, 404, 'No user with that email');
    const temp = tempPassword();
    await setPassword(u, temp);
    console.log('[auth] temporary password issued for', u.email);
    ok(res, { email: u.email, temp });
  } catch (e) { bad(res, 400, e.message); }
});
