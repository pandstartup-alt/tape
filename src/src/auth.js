...
import { createUser, findUserByEmail, findUserById, isPaid, save } from './store.js';
...
/* Replace a password. Used both by a member changing their own and by the
   admin issuing a temporary one when someone is locked out. */
export async function setPassword(user, next) {
  next = String(next || '');
  if (next.length < 8) throw new Error('Password must be at least 8 characters');
  user.hash = await bcrypt.hash(next, 10);
  save();
  return user;
}

export const checkPassword = (user, password) =>
  bcrypt.compare(String(password || ''), user.hash);

/* Readable temporary password — no ambiguous characters, since you will be
   typing this to someone over chat. */
